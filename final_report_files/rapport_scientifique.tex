...
\begin{itemize}
    \item Pour HomeStrength: $S_{max} = \sum_{i=1}^{n} \sum_{j=i+1}^{n} (j-i) / 2$. C'est une estimation de la valeur maximale possible, considérant qu'environ la moitié des matchs contre des plus faibles sont joués à domicile. Le calcul exact dépend du calendrier, mais cette valeur fournit un ordre de grandeur pour la normalisation. Une analyse plus fine du $S_{max}$ théorique pour cette nouvelle définition serait nécessaire pour une normalisation parfaite.
