...
\begin{itemize}
    ...
    \[
    \frac{MD_{max}}{\mu_{MD}} \approx \frac{\sqrt{n-1}}{\sqrt{2 \ln n}} \quad \Rightarrow \quad \text{ce ratio diverge avec } n.
    \]
\end{itemize}
...
Chaque algorithme est lancé 10 fois avec une limite de temps stricte de \textbf{60 secondes}.  
Les meilleures solutions trouvées lors de chaque exécution sont évaluées via les trois métriques normalisées analytiquement.
...
On constate que la variante \textit{Opti} domine systématiquement la version \textit{Non‑Opti} sur les trois métriques, avec un avantage qui s’amplifie lorsque la taille du tournoi augmente. Les tendances théoriques $O(n^{3})$ pour HS, $O(n^{2})$ pour PS et $O(\sqrt{\ln n})$ pour MD se vérifient empiriquement.
