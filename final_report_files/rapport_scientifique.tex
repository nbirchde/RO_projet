...
\begin{equation}
    ...
    \end{cases}
\end{equation}
où $P_{eff} = \{1, \dots, n_{eff}\}$ est l'ensemble des participants (avec $i \neq j$) et $R = \{0, \dots, n_{eff}-2\}$ représente l'ensemble des rondes. L'indice du joueur correspond à son classement (1 étant le plus fort).
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Lien $MaxDev$ (Écart Max Domicile) :} Calcule l'écart maximal par rapport à la moyenne $\bar{H} = \frac{n_{eff}-1}{2}$.
    \begin{align}
...
Les solveurs minimisent ensuite la somme pondérée des versions sigmoïdes (variable \verb|scaled_score|), ce qui rend les trois critères directement comparables même pour un lectorat peu familier avec les statistiques.
où $M_k^{raw}$ est la valeur brute de la métrique $k \in \{\mathrm{HS, PS, MD}\}$.
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        \sigma_{PS} &= \sqrt{n \cdot \frac{n-2}{2}} \quad \text{si } n > 2, \text{ sinon } 1.0
    \end{align*}
    ...
    \begin{align*}
        ...
        \sigma_{MD} &\approx \frac{\sqrt{n-1}}{2 \sqrt{2 \ln n}}
    \end{align*}
