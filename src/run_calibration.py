...
RUNS_PER_COMBINATION = 8 # Number of parallel SA chains for each (alpha, beta)
...
if __name__ == '__main__':
        ...
        for beta_raw in BETA_VALUES:
            ...
            try:
                ...
                if RUNS_PER_COMBINATION > 1:
                    # solve_sa_parallel returns: best_schedule, best_norm_score (analytical), raw_metrics, analytical_metrics (z-scores), scaled_score, scaled_metrics ([0,1])
                    ...
                    # solve_sa returns: best_schedule, best_norm_score (analytical), raw_metrics, analytical_metrics (z-scores), scaled_score, scaled_metrics ([0,1])
                ...
                if not best_schedule_list:
                    ...
                    results.append({
                        ...
                        "total_analytical_score": np.nan, # This is the original z-score sum
                        "scaled_hs": np.nan, "scaled_ps": np.nan, "scaled_md": np.nan,
                        "total_scaled_score": np.nan, # This is the new scaled score sum
                ...
                # Scaled metrics ([0,1]) from the SA solver
                scaled_hs, scaled_ps, scaled_md = scaled_metrics

                # The total_analytical_score is the original z-score sum
                total_analytical_score = analytical_obj_score # Already returned by solver

                # The total_scaled_score is the new scaled score sum
                total_scaled_score = scaled_obj_score # Already returned by solver


                results.append({
                    "alpha": alpha,
                    "beta": beta,
                    "anal_norm_hs": anal_norm_hs, # Analytically normalized HS (Z-score)
                    "anal_norm_ps": anal_norm_ps, # Analytically normalized PS (Z-score)
                    "anal_norm_md": anal_norm_md, # Analytically normalized MD (Z-score)
                    "total_analytical_score": total_analytical_score, # Original Z-score sum
                    "scaled_hs": scaled_hs, # Scaled HS ([0,1])
                    "scaled_ps": scaled_ps, # Scaled PS ([0,1])
                    "scaled_md": scaled_md, # Scaled MD ([0,1])
                    "total_scaled_score": total_scaled_score, # New scaled score sum
    ...
    if results:
        ...
        column_order = [
            "alpha", "beta", "status", 
            "anal_norm_hs", "anal_norm_ps", "anal_norm_md", # Analytically normalized (Z-scores)
            "total_analytical_score", # Original Z-score sum
            "scaled_hs", "scaled_ps", "scaled_md", # Scaled metrics ([0,1])
            "total_scaled_score", # New scaled score sum
